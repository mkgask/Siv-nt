...
export default class Media {
    ...
    generateDefaultInfo(
        ...
        if (!mime_type) {
            ...
            log.debug('media', 'Media.generateDefaultInfo(): mime.getType(path): ', this.mime_type)
        } else {
        ...
        if (!type) {
            ...
            log.debug('media', 'Media.generateDefaultInfo(): accepted_types[this.mime_type]: ', this.type)
        } else {
            this.type = type
        }
        ...
        if (!time) {
            ...
            log.debug('media', 'Media.generateDefaultInfo(): statSync(path).mtimeMs: ', this.time)
        } else [
            this.time = time
        ]
        ...
        if (!filesize) {
            ...
            log.debug('media', 'Media.generateDefaultInfo(): calculateDisplayFileSize(statSync(path).size): ', this.filesize)
        } else {
